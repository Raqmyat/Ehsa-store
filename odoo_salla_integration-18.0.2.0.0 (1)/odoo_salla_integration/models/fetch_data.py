...
class FetchData:
    ...
    def get_product_basic_vals(self, product):
        return {
            ...
            # 'standard_price': product.get('cost_price').get('amount') if product.get('cost_price') else 0.0,
    ...
    def get_variant_vals(self, product):
        ...
        for variant in product.get('skus'):
            ...
            variants.append({
                ...
                'standard_price':variant.get('cost_price').get('amount'),
    ...
    def process_order(self, order):
        ...
        cash_on_delivery = order.get('amounts',{}).get('cash_on_delivery',{})
        if cash_on_delivery:
    ...
    def get_variant_id(self, order_line_product, attribute_options=False):
        ...
        for rec in order_line_product.get("options"):
            val = rec.get('value')
            res_val = val.get("id") if isinstance(val, dict) else val
            if res_val is not None:
                option_vals.append(res_val)
        option_vals = [str(val) for val in option_vals]
        option_vals.sort()
        value = "_".join(map(str, option_vals))
        mappings = self.channel_id.match_template_mappings(
            store_product_id=order_line_product.get("product").get("id") if order_line_product.get("product") else order_line_product.get("product_id")) 
        if mappings:
