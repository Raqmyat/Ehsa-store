    ...
    def get_orders(self, **kw):
        ...
        if kw.get('filter_type') == "id":
            ...
            for order_store_id in order_store_ids:
                if order_store_id:
                    endpoint = self.import_url + "orders/" + order_store_id +'?format=light'
                    response = self.salla_response(endpoint, params=params)
                    if response:
                        data = response.get('data')
                        # order_data = self.fetch_data().process_order(response.get('data'))
                        params = {}
                        endpoint = self.import_url + "shipments?order_id=" + order_store_id
                        response_shipments = self.salla_response(endpoint, params=params)
                        if response_shipments and response_shipments.get('data'):
                            data_shipments = response_shipments.get('data')
                            data.update(
                                    {'shipments':data_shipments})
                        endpoint_items = self.import_url + "orders/items?order_id=" + str(order_store_id)
                        response_items = self.salla_response(endpoint_items, params=params)
                        if response_items and response_items.get('data'):
                            items_data = response_items.get('data')
                            data.update(
                                    {'items':items_data})
                                           
                        order_data = self.fetch_data().process_order(data)
